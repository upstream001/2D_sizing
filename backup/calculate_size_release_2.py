...
"""
#可以计算出上下两个端点到中间线的距离，同时可以计算出中间线的长度
...
def calculate_strawberry_size(image_path, save_results=True, output_dir="results", 
    ...
    # 9. 输出尺寸信息摘要
    print("\n=== 草莓尺寸计算结果摘要 ===")
    for i, (pc1_data, line_data, perp_data, real_distance_data) in enumerate(zip(pc1_endpoints_list, line_results_list, perpendicular_distance_results, real_distance_results)):
        print(f"草莓 {i+1}:")
